...
// 4668 - 'x' is not defined as a preprocessor macro, replacing with '0' for
// '#if/#elif'
...
// MSVC++ 14.2 (16) _MSC_VER == 1920 (Visual Studio 2019) << NOT YET RELEASED -
// April 2 2019
...
struct DOCTEST_INTERFACE IReporter {
  ...
  // filters, has a skip decorator
  // or isn't in the execution range (between first and last) (safe to cache a
...
#define DOCTEST_TEST_CASE_TEMPLATE_DEFINE_IMPL(dec, T, iter, func) \
  ...
  struct iter<std::tuple<Type, Rest...>> {                         \
    iter(int line, int index) {                                    \
      DOCTEST_REGISTER_TYPED_TEST_CASE_IMPL(func<Type>, Type, dec, \
                                            line * 1000 + index);  \
...
#define DOCTEST_TEST_CASE_TEMPLATE_INVOKE_IMPL(id, anon, ...)         \
  DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_CAT(anon, DUMMY)) = [] {         \
    DOCTEST_CAT(id, ITERATOR)<std::tuple<__VA_ARGS__>> DOCTEST_UNUSED \
        DOCTEST_CAT(anon, inner_dummy)(__LINE__, 0);                  \
    return 0;                                                         \
  }();                                                                \
  DOCTEST_GLOBAL_NO_WARNINGS_END()

#define DOCTEST_TEST_CASE_TEMPLATE_INVOKE(id, ...)            \
  DOCTEST_TEST_CASE_TEMPLATE_INVOKE_IMPL(                     \
...
#define DOCTEST_TEST_CASE_TEMPLATE_APPLY(id, ...)             \
  DOCTEST_TEST_CASE_TEMPLATE_APPLY_IMPL(                      \
      id, DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_), __VA_ARGS__) \
  typedef int DOCTEST_ANONYMOUS(_DOCTEST_ANON_FOR_SEMICOLON_)
...
// register a
// reporter with a duplicate name and a different priority but hopefully that
// won't happen often :|
